      ...
      if (seq[1] >= '0' && seq[1] <= '9') {
        ...
        if (seq[2] == '~') {
          switch (seq[1]) {
            ...
            case '3': return DEL_KEY;  // macos/iterm fn+delete
        ...
        switch (seq[1]) {
          ...
          case 'H': return HOME_KEY;  // macos/iterm fn+left
          case 'F': return END_KEY;   // macos/iterm fn+right
        }
      }
    } else if (seq[0] == 'O') {
      switch (seq[1]) {
...
void editorInsertChar(int c) {
  if (E.cy == E.numrows) { // cursor is on the ~ line after the end of the file
    editorInsertRow(E.numrows, "", 0); // start a new row
  }
...
char *editorRowsToString(int *buflen) {
  ...
  for (j = 0; j < E.numrows; j++) {
    totlen += E.row[j].size + 1; // +1 for the newline we'll be appending
  }
  *buflen = totlen;

  // copy the rows' contents into a buffer
...
void editorDrawStatusBar(struct abuf *ab) {
  abAppend(ab, "\x1b[7m", 4); // set inverted color
  ...
  abAppend(ab, "\x1b[m", 4); // set inverted color
  abAppend(ab, "\r\n",2); // line 2 of the status bar
}
...
void initEditor() {
  ...
  E.screenrows -= 2; // 2 lines held back for the status bar
}
